<template>
    ...
    >
        <div v-if="is_authenticated()">
            <div v-if="full_name != ''" class="font-semibold text-sm">{{ full_name }}</div>
            <div v-if="username != ''" class="font-light text-sm mb-3">{{ username }}</div>

            <Button  class="w-full mb-3" severity="secondary" @click="this.$router.push('/auth/login');" style="justify-content: space-between;"  outlined> 
                <span class="p-button-label px-2 py-1" style="color: var(--p-button-outlined-secondary-color);">Sair</span>
                <div class="mx-4">
        ...
        <Panel toggleable>
            <template #header>
                <span class="p-button-label" style="color: var(--p-button-outlined-secondary-color);">Personalizar</span>
            </template>
            <template #toggleicon>
                <Button icon="pi pi-palette" severity="secondary" rounded text @click="toggle" />
            </template>
            <div class="flex flex-col gap-4">
                <div>
                    <span class="text-sm text-muted-color font-semibold">Cor Principal</span>
                ...
                <div>
                    <span class="text-sm text-muted-color font-semibold">Cor de Fundo</span>
                    <div class="pt-2 flex gap-2 flex-wrap justify-start">
                        <button
...
export default {
    ...
    data() {
        ...
        return {
            ...
            preset,
            presetOptions,
            menuMode: ref(layoutConfig.menuMode),
            menuModeOptions: ref([
    ...
    methods: {
        getPresetExt() {
            const color = this.primaryColors.find((c) => c.name === this.layoutConfig.primary);
        ...
        updateColors(type, color) {
            if (type === 'primary') {
                this.setPrimary(color.name);
            } else if (type === 'surface') {
                this.setSurface(color.name);
            }
        ...
        onPresetChange() {
            ...
            const presetValue = this.presets[this.preset];
            const surfacePalette = this.surfaces.find((s) => s.name === this.layoutConfig.surface)?.palette;

            $t().preset(presetValue).preset(this.getPresetExt()).surfacePalette(surfacePalette).use({ useDefaultOptions: true });
        },
        onMenuModeChange() {
            this.setMenuMode(this.menuMode);
        },
        is_authenticated() {
    ...
    created() {
        if (this.layoutConfig.primary) {
            this.updateColors('primary', this.primaryColors.find((c) => c.name === this.layoutConfig.primary));
        }

        if (this.layoutConfig.surface) {
            this.updateColors('surface', this.surfaces.find((c) => c.name === this.layoutConfig.surface));
        }

        if (this.layoutConfig.preset) {
            this.onPresetChange();
        }

        if (this.layoutConfig.menuMode) {
            this.onMenuModeChange();
        }
