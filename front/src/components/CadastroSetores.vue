<template>
    <DataTable v-model:filters="filters" :value="departments" paginator :loading="loading" :rows="5"
        ...
        <template #empty> Nenhum setor encontrado. </template>
        <template #loading> Carregando Setores. Por favor aguarde. </template>
        ...
        <Column style="width: 10%; ">
            <template #header="slotProps">
                <div class="flex flex-wrap justify-end w-full">
                    <Button icon="pi pi-plus" label="Novo Setor" @click="activateAddEditor" />
                </div>
            </template>
            <template #body="slotProps">
                <div class="flex justify-end">
                    <Button @click="activateEditEditor(slotProps.data)" icon="pi pi-pencil" text aria-label="Edit" />
                    <Button @click="deleteDepartment(slotProps.data)" icon="pi pi-trash" severity="danger" text
                        aria-label="Delete" />
                </div>
            </template>
        </Column>
...
import { handleDepartmentErrors } from '@/common/utils';
    ...
    methods: {
        ...
        activateAddEditor() {
            this.editor = {
                ...
                mode: 'add',
                header: 'Novo Setor',
                help: 'Informe o nome do setor que deseja cadastrar e sua respectiva cor'
            }
        },
        activateEditEditor(department) {
            this.editor = {
                visible: true,
                closable: true,
                loading: false,
        ...
        editDepartment() {
            ...
            api.postEditDepartment(data, this.editor.id)
                .then(response => {
                    this.editor.closable = true;
                    this.editor.visible = false;
                    this.clearDepartments();
        ...
        deleteDepartment(department) {
            this.$confirm.require({
                message: `Você tem certeza de que deseja excluir o setor ${this.formatNome(department.name)}?`,
                ...
                accept: () => {
                    ...
                    api.deleteDepartment(department.id)
                        .then(response => {
                            this.clearDepartments();
                        })
                        .catch(error => {
                    ...
                    this.$toast.add({ severity: 'info', summary: 'Exclusão Confirmada', detail: `Setor ${this.formatNome(department.name)} excluído com sucesso`, life: 10000 });
                },
                reject: () => {
                    this.$toast.add({ severity: 'warn', summary: 'Ação Cancelada', detail: `Você desistiu da exclusão do setor ${this.formatNome(department.name)}`, life: 10000 });
                }
            });
        },
    },
